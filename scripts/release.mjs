...
if (!match) {
    throw new Error(`La versión actual no es válida: ${packageJson.version}`);
}

const nextVersion = `${match[1]}.${Number(match[2]) + 1}.0`;
const tag = `v${nextVersion}`;
const existingTag = git(["tag", "--list", tag], { capture: true });
...
if (dryRun) {
    ...
    console.log(`Todo correcto: npm run minor publicaría ${tag}, el bundle itch.io y el instalador Electron.`);
    process.exit(0);
}
...
console.log(`${tag} publicado en GitHub; el workflow del tag publicará npm y creará el release.`);
